...
export function TredingComponent ({coin}) {
    ...
    /* console.log(market)
    const dls_current = (market.length > 0) ? market.filter(value => value.id === id) : null */
    /* console.log(dls_current)     */
    return (
            <Link to={`/search/${id}`}>
                ...
                <div className={`flex flex-col text-right justify-center gap-2`}>
                    <span className={`text-${colors.color_secondary}`}>Not found value</span>
...
{/* <span className={`text-${colors.color_secondary}`}>{dls_current === null ? "Not found value" : `U$ ${dls_current[0].current_price}`}</span> */}